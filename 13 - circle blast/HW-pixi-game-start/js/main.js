...
let gameOverScoreLabel,gameOverText,playAgainButton;
...
function loadLevel(){
    ...
    if (tripleFire != true && score >= 5) tripleFire = true;
	paused = false;
}
...
function fireBullet(e){
    ...
    if (tripleFire){
        let bLeft = new Bullet(0xFFFFFF,ship.x-8,ship.y);
        let bRight = new Bullet(0xFFFFFF,ship.x+8,ship.y);
...
function createExplosion(x,y,frameWidth,frameHeight){
    ...
    expl.x = x - w2; // we want the explostions to appear at the center of ther circle
    expl.y = y - h2; // ditto
...
function showFinalScore(score){
    let gameOverScoreLabel = new PIXI.Text("Your final score: " + score);
    let textStyle = new PIXI.TextStyle({
        fill: 0xFFFFFF,
	    fontSize: 32,
	    fontFamily: "Futura",
